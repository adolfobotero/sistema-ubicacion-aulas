...
const MoverAsignatura = ({ setActiveSection }) => {
  ...
  const handleSeleccionAsignatura = async (compositeValue) => {
    try {
      const seleccion = JSON.parse(compositeValue);
      setSelectedValue(compositeValue);

      const encontrada = asignaturas.find(
        a =>
          a.idasignatura === seleccion.idAsignatura &&
          a.idaula === seleccion.idAula &&
          a.diasemana === seleccion.diaSemana &&
          a.horainicio === seleccion.horaInicio &&
          a.horafin === seleccion.horaFin
      );

      setAsignaturaSeleccionada(encontrada || null);
      setNuevaAula('');
      setNuevoDia('');
      setNuevaHoraInicio('');
      setNuevaHoraFin('');
      ...
      if (encontrada) {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/asignaturas/${encontrada.idasignatura}/profesores`);
        const data = await res.json();
        setProfesoresAsignatura(data);
      }
    } catch (err) {
      console.error('Error al procesar la selección:', err);
    ...
    <div className="form-container">
      ...
      <div className="form-flex">
        ...
        >
          ...
          {asignaturas.map(a => (
            <option
              key={`${a.idasignatura}-${a.idaula}-${a.diasemana}-${a.horainicio}`}
