...
const MoverAsignatura = ({ setActiveSection }) => {
  ...
  const handleSeleccionAsignatura = async (compositeValue) => {
    ...
    const [id, dia, inicio, fin] = compositeValue.split('|');

    const seleccionada = asignaturas.find(
      a =>
        a.idasignatura.toString() === id &&
        a.diasemana === dia &&
        a.horainicio === inicio &&
        a.horafin === fin
    );
    ...
    if (seleccionada) {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/asignaturas/${seleccionada.idasignatura}/profesores`);
    ...
    <div className="form-container">
      ...
      <div className="form-flex">
        ...
        >
          ...
          {asignaturas.map(a => (
            <option
              key={`${a.idasignatura}-${a.diasemana}-${a.horainicio}-${a.horafin}`}
              value={`${a.idasignatura}|${a.diasemana}|${a.horainicio}|${a.horafin}`}
            >
